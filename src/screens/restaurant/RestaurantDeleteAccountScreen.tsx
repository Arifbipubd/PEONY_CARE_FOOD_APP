...
type Props = {
  navigation: NativeStackNavigationProp<ProfileStackParamList, 'RestaurantDeleteAccount'>;
};

const DELETED_ITEMS = [
  'Your restaurant profile, address, hours, and dish photos',
  'UEN and contact details (kept 90 days for tax records)',
  'All donation history and analytics',
  'Any active donations (closed automatically)',
  'Sponsored donation payout records (kept 7 years per ACRA)',
] as const;

export default function RestaurantDeleteAccountScreen({ navigation }: Props) {
  ...
  const handleDelete = useCallback(async () => {
    ...
    setError('');
    try {
      await api.post('/restaurant/account/delete/', { confirmation: 'DELETE' });
      clearAuth();
    } catch (err: unknown) {
  ...
  return (
    <SafeAreaView style={styles.screen}>
      ...
      >
        ...
        <Text style={styles.title}>Delete restaurant account</Text>

        <Text style={styles.desc}>
          This is permanent. Your business profile and history can't be recovered.
        </Text>
        ...
        >
          {loading ? (
            ...
            <>
              ...
              <Text style={styles.deleteBtnLabel}>Delete restaurant account</Text>
            </>
          )}
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  confirmInputFocused: { borderColor: colors.dangerRed },
